...
def main(seed):
    ...
    if loss[-1] < ground_energy + 0.5:
        np.save(f'Result\Result_for_Proposed_Circuit_all_nodes\{nodes_pool[index]}\Loss\{qubits_num}_qubits_model_loss_{seed}.npy',loss)
        np.save(f'Result\Result_for_Proposed_Circuit_all_nodes\{nodes_pool[index]}\Converge_ITR\{qubits_num}_qubits_model_converge_Itr_{seed}.npy',converge_Itr)   
