...
class DistributedVQE_nparts(torch.nn.Module):
    ...
    def __init__(self, num_qubit, num_part, cir_depth, hamiltonian_list, seed,ansatz):
        ...
        for i in range(num_part):
            ...
            if ansatz == 'HEA1':
                circuit = build_HEA1_circuit(qubit_num=len(sub_list[i][0][0]), circuit_depth=cir_depth)
            elif ansatz == 'HEA2':
                circuit = build_HEA2_circuit(qubit_num=len(sub_list[i][0][0]), circuit_depth=cir_depth)
            else:
                circuit = build_proposed_circuit(qubit_num=len(sub_list[i][0][0]), circuit_depth=1)
