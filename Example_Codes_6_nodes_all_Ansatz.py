...
def main(seed):
    for ansatz in ansatz_pool:
        ...
        if loss[-1] < ground_state_energy + 10000:
                np.save(f'Result/Result_for_6_nodes_all_Ansatz/{ansatz}/Loss/{qubits_num}_qubits_model_loss_{seed}.npy',loss)
                np.save(f'Result/Result_for_6_nodes_all_Ansatz/{ansatz}/Converge_ITR/{qubits_num}_qubits_model_converge_Itr_{seed}.npy',converge_Itr)   
